...
from global_variables import UNKNOWN_WORD, END_WORD, NAN_WORD, LIST_CLASSES, LIST_LOGITS, COMMENT, VALID_SLIM_FILENAME, TRAIN_SLIM_FILENAME, TEST_FILENAME
...
if VALID_DATA_FN is not None:
    valid_data = pd.read_csv(VALID_DATA_FN)
    if cfg.do_preprocess:
...
def fold_predicts(predicts_list):
...
def populate_submission(test_results, valid_results):
    ...
    if MODE == 'train':
        ...
        oob_validation_data = valid_data.copy()
        oob_validation_data.drop(columns=["comment_text"])
        oob_validation_data[LIST_LOGITS] = pd.DataFrame(valid_results, index=oob_validation_data.index)


        submission_folder = 'submissions/'
        submission = pd.read_csv("assets/raw_data/sample_submission.csv")
        submission[LIST_CLASSES] = test_results
    if not os.path.exists(root + model + submission_folder):
        os.mkdir(root + model + submission_folder)
    fn = root + model + submission_folder
    fn += model[:-1] + epoch + '.csv'
    submission.to_csv(fn, index=False)


new_submission = valid_data.copy()
new_submission[LIST_LOGITS] = test_predicts

new_submission.to_csv(root + model + "l2_valid_data.csv", index=False)

new_submission = pd.read_csv("assets/raw_data/sample_submission.csv")
new_submission[LIST_CLASSES] = test_predicts
new_submission.to_csv(root + model + model[:-1] + "_test_data_folded.csv", index=False)