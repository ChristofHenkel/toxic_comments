...
list_sentences = train["comment_text"].fillna("Nothing").values
...
fold_size = len(list_sentences) // fold_count
results = []
for fold_id in range(0, fold_count):
    ...
    if fold_id == fold_size - 1:
        fold_end = len(list_sentences)

    list_sentences_valid = list_sentences[fold_start:fold_end]
    Y_valid = Y[fold_start:fold_end]

    list_sentences_train = np.concatenate([list_sentences[:fold_start], list_sentences[fold_end:]])
    ...
    tokenizer = Tokenizer(min_count_words=5,min_count_chars=100)
...
sample_submission.to_csv("submissions/XGZ_v1_10fold_on_e1.csv", index=False)
